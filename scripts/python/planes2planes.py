...
# Tool to extract individual planes from plane files
# To be checked in new version
    ...
    for var in range(sizeofvars):
        for index, plane in enumerate(setofplanes):
            if plane in selectedplanes:
                fin.seek(
                    sizeofheader + (index + var * len(setofplanes)) * sizeofplane * sizeofdata,
                    0,
                )
                    ...
                    case "NetCDF":
                        match planetype:
                            case "xy":
                                za = np.array(
                                    [z[plane - 1]]
                                )  # Note that the plane starts at 1 and not 0
                                nz = len(za)
                            case "xz":
                            ...
                            case "yz":
                                ...
                                nx = len(xa)
                        a = np.array(
                            struct.unpack((etype + "{}" + dtype).format(int(nx * ny * nz)), raw)
                        )
