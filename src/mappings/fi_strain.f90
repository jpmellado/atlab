...
! Uz, Wx
        ...
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, u, tmp5)
        tmp5 = 0.5_wp*(tmp5 + tmp1)

! Vz, Wy
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, w, tmp1)
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, v, tmp6)
...
! S_11 = U,x
! -------------------------------------------------------------------
        call OPR_Partial_X(OPR_P1, nx, ny, nz, u, strain)

        call OPR_Partial_Z(OPR_P2, nx, ny, nz, strain, tmp3, tmp4)
        call OPR_Partial_Y(OPR_P2, nx, ny, nz, strain, tmp2, tmp4)
        call OPR_Partial_X(OPR_P2, nx, ny, nz, strain, tmp1, tmp4)
...
! S_22 = V,y
! -------------------------------------------------------------------
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, v, strain)
...
! S_33 = W,z
! -------------------------------------------------------------------
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, w, strain)
...
! S_13 = (U,z + W,x)/2
! -------------------------------------------------------------------
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, u, tmp1)
        call OPR_Partial_X(OPR_P1, nx, ny, nz, w, tmp2)
        strain = tmp1 + tmp2
...
! -------------------------------------------------------------------
        call OPR_Partial_X(OPR_P1, nx, ny, nz, u, tmp1)
        call OPR_Partial_X(OPR_P2, nx, ny, nz, p, tmp2, tmp4)
        result = tmp1*tmp2

        call OPR_Partial_Y(OPR_P1, nx, ny, nz, v, tmp1)
        call OPR_Partial_Y(OPR_P2, nx, ny, nz, p, tmp2, tmp4)
        result = result + tmp1*tmp2

        call OPR_Partial_Z(OPR_P1, nx, ny, nz, w, tmp1)
        call OPR_Partial_Z(OPR_P2, nx, ny, nz, p, tmp2, tmp4)
        result = result + tmp1*tmp2
...
! -------------------------------------------------------------------
! 2 s_12 p,xy
        call OPR_Partial_X(OPR_P1, nx, ny, nz, p, tmp2)
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, tmp2, tmp1)
        call OPR_Partial_X(OPR_P1, nx, ny, nz, v, tmp2)
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, u, tmp3)
        result = result + tmp1*(tmp2 + tmp3)

! 2 s_13 p,xz
        call OPR_Partial_X(OPR_P1, nx, ny, nz, p, tmp2)
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, tmp2, tmp1)
        call OPR_Partial_X(OPR_P1, nx, ny, nz, w, tmp2)
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, u, tmp3)
        result = result + tmp1*(tmp2 + tmp3)

! 2 s_23 p,yz
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, p, tmp2)
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, tmp2, tmp1)
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, w, tmp2)
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, v, tmp3)
        result = result + tmp1*(tmp2 + tmp3)
...
!########################################################################
    subroutine FI_STRAIN_A(nx, ny, nz, a, u, v, w, strain1, strain2, normal1, normal2, normal3, tmp1)
        ...
        ! ###################################################################
        ! Compute normals
        call OPR_Partial_X(OPR_P1, nx, ny, nz, a, normal1)
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, a, normal2)
        call OPR_Partial_Z(OPR_P1, nx, ny, nz, a, normal3)

        ! Compute gradient terms with u
        call OPR_Partial_X(OPR_P1, nx, ny, nz, u, tmp1)
        strain1 = normal1*tmp1*normal1
        call OPR_Partial_Y(OPR_P1, nx, ny, nz, u, tmp1)
