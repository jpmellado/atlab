...
subroutine NSE_Anelastic_PerVolume()
    ...
    call NSE_AddBurgers_PerVolume_Z(0, imax, jmax, kmax, w, hq(:, 3), tmp1, rhou_out=tmp6)          ! store rho w in tmp6

    ! Ox momentum equation
    call NSE_AddBurgers_PerVolume_Y(0, imax, jmax, kmax, u, hq(:, 1), tmp1, tmp2, rhou_in=tmp5)     ! tmp5 contains rho v transposed
    call NSE_AddBurgers_PerVolume_Z(0, imax, jmax, kmax, u, hq(:, 1), tmp1, rhou_in=tmp6)           ! tmp6 contains rho 3 transposed

    ! Oy momentum equation
    call NSE_AddBurgers_PerVolume_X(0, imax, jmax, kmax, v, hq(:, 2), tmp1, tmp2, rhou_in=tmp4)     ! tmp4 contains rho u transposed
    call NSE_AddBurgers_PerVolume_Z(0, imax, jmax, kmax, v, hq(:, 2), tmp1, rhou_in=tmp6)           ! tmp6 contains rho 3 transposed
    ...
    do is = 1, inb_scal
        ...
        call NSE_AddBurgers_PerVolume_Z(is, imax, jmax, kmax, s(:, is), hs(:, is), tmp1, rhou_in=tmp6)          ! tmp6 contains rho 3 transposed
